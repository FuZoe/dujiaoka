...
function pubOrder(o){return{amountFen:o.amountFen,createdAt:o.createdAt,id:o.id,paidAt:o.paidAt||null,returnUrl:o.returnUrl||'',source:o.source,status:o.status,title:o.title,payee:o.payee||''}}
...
for(const order of state.orders){const expectedPayee=order.source==='dujiaoka'?'admin':(order.payee||'admin');if(order.payee!==expectedPayee){order.payee=expectedPayee;stateMigrated=true}}
...
if(!/^[A-Z0-9_-]{8,64}$/.test(id)||!Number.isInteger(af)||af<1||af>99999999)return sendJson(res,400,{error:'invalid_order'});
...
if(!order){order={id,createdAt:new Date(now()).toISOString()};state.orders.push(order)}
Object.assign(order,{amountFen:af,callbackStatus:order.callbackStatus||'waiting',callbackUrl:cbUrl,returnUrl:retUrl,source:'dujiaoka',status:'pending',title,payee:'admin',updatedAt:new Date(now()).toISOString()});
persist();
return sendJson(res,201,{order:pubOrder(order),paymentUrl:'https://pay.newzoe.cloud/'+id});
}

async function cbShop(order,tid){
if(order.source!=='dujiaoka'||!order.callbackUrl||!shopSecret)return;
const pl=JSON.stringify({amountFen:order.amountFen,orderId:order.id,paidAt:order.paidAt,transactionId:tid});
const ts=String(now());
try{const r=await fetchImpl(order.callbackUrl,{body:pl,headers:{'content-type':'application/json','x-shop-signature':signPayload(shopSecret,ts,Buffer.from(pl)),'x-shop-timestamp':ts},method:'POST'});order.callbackStatus=r.ok?'success':'http_'+r.status}catch(e){order.callbackStatus='error';console.error('cb fail',order.id,e.message)}
order.updatedAt=new Date(now()).toISOString();persist()
}

async function settleOrder(order,paidAt,tid){
if(order.status==='paid')return{duplicate:true,delivered:0};
order.status='paid';order.paidAt=paidAt;order.transactionId=tid;
order.updatedAt=new Date(now()).toISOString();
state.txns.push({amountFen:order.amountFen,orderId:order.id,paidAt,transactionId:tid});
...
const merged=shop.map(so=>{const po=localById.get(so.id);localById.delete(so.id);return{...so,payee:'admin',payment:po?pubOrder(po):null}});
for(const o of localById.values())merged.push({amountFen:o.amountFen,createdAt:o.createdAt,id:o.id,source:o.source,status:o.status,title:o.title,payee:orderPayee(o),payment:pubOrder(o)});
...
if(!af||af>99999999||!title)return sendJson(res,400,{error:'invalid_order'});
const id='M'+new Date(now()).toISOString().replace(/\D/g,'').slice(2,14)+crypto.randomBytes(3).toString('hex').toUpperCase();
const order={amountFen:af,createdAt:new Date(now()).toISOString(),id,source:'manual',status:'pending',title,payee:curObj.username,updatedAt:new Date(now()).toISOString()};
...
const co=oid?orderById(oid):null;sendEvent(res,'status',co?pubOrder(co):{status:'waiting'});
...
if(oid){const order=orderById(oid);if(!order)return sendJson(res,404,{error:'order_not_found'});payee=orderPayee(order)}
