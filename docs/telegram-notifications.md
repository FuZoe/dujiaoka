...
- `Telegram目标频道` 只接收商品补货公告，不接收订单号、邮箱、卡密或其他顾客数据。
...
2. 至少授予机器人“发布消息”权限。
3. 公开频道在后台 `系统设置 -> 订单推送配置 -> Telegram目标频道` 填写完整用户名，例如 `@channel_username`。
4. 私有频道填写字符串形式的数值 chat_id，通常以 `-100` 开头，例如 `-1001234567890`。
5. 获取私有频道 ID 时，可先让机器人收到频道消息，再调用 Bot API `getUpdates`，查找 `channel_post.chat.id`；也可把频道消息转发给可显示原始 chat_id 的工具机器人。
6. 用 `php artisan telegram:restock-test` 发送测试公告。文案固定包含“补货通知测试”。
